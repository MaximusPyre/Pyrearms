...
import { BODYCAM_STATES } from "../data/bodycamStates";
		...
		<section className="page">
			<div className="page-head">
				...
				<p className="lede">
					...
					how states differ. Pick a state guide when you know the agency.
					Education, not legal advice.
				</p>
			</div>

			<div className="bodycam-states">
				{BODYCAM_STATES.map((state) => {
					const inner = (
						<>
							<div className="product-card-top">
								<span>{state.status === "ready" ? "Guide" : "Soon"}</span>
					...
					return (
						<article className="product-card" key={state.slug}>
							{state.status === "ready" ? (
								<Link
									className="product-card-link"
									to={`/bodycams/${state.slug}`}
								>
									{inner}
								</Link>
							) : (
								<div className="product-card-link">{inner}</div>
							)}
						</article>
					);
				})}
			...
			<div className="prose">
				...
				<ol>
					...
					<li>
						...
						right statute — almost never federal FOIA for local cops. In
						Pennsylvania, that statute is{" "}
						<Link to="/bodycams/pennsylvania">
							Act 22 / 42 Pa.C.S. Chapter 67A
						</Link>
						, not the Right-to-Know Law.
					</li>
					<li>
				...
				<p>
					...
					Act, Florida Sunshine Law. Pennsylvania’s general records law is
					Right-to-Know, but bodycam and dashcam go through{" "}
					<Link to="/bodycams/pennsylvania">Act 22</Link>.
				</p>
				<p>
				...
				<p>
					...
					formal litigation hold. This page is not a substitute for that. In
					Pennsylvania, Act 22’s preservation duty does not start until a valid
					request is actually received — see the{" "}
					<Link to="/bodycams/pennsylvania">state guide</Link>.
				</p>
				...
				<p>
					...
					<strong>
						Pennsylvania is the exception — email and portals do not count.
					</strong>{" "}
					Use the{" "}
					<Link to="/bodycams/pennsylvania">Act 22 form creator</Link>.
				</p>
...
[Phone]`}</pre>
				<p>
					Do not use that letter for Pennsylvania police video. Use the{" "}
					<Link to="/bodycams/pennsylvania">Act 22 walkthrough</Link> and
					deliver it in person or by certified mail.
				</p>
				...
				<ul>
					<li>
						...
						requesters). Pennsylvania is in this bucket —{" "}
						<Link to="/bodycams/pennsylvania">full Act 22 process</Link>.
					</li>
					<li>
				...
				<ol>
					...
					<li>
						...
						depending on the state. Pennsylvania’s path is the Act 22 petition
						on the <Link to="/bodycams/pennsylvania">state page</Link>.
					</li>
					<li>
